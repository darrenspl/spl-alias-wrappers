...
```
.aichats/2025-01-14-01-initial-setup.md  ← Single file per session
```

**OLD WAY (v2.0) - CAUSED SUMMARIZATION:**
```
.aichats/YYYY-MM-DD-01-initial-setup/    ← FOLDER per session
...
1. **Check for existing session folder:**
   - Look in `.aichats/` for folders matching today's date pattern: `YYYY-MM-DD-*`
...
2. **Create or use folder:**
   - If no matching folder exists, create: `.aichats/YYYY-MM-DD-XX-description/`
...
```
.aichats/YYYY-MM-DD-XX-description/
...
### ✅ Correct Examples:
```
.aichats/YYYY-MM-DD-01-initial-setup/
.aichats/YYYY-MM-DD-02-database-migration/
.aichats/YYYY-MM-DD-01-api-integration/
```

### ❌ WRONG Examples:
```
.aichats/YYYY-MM-DD-01-initial-setup.md  ← NO! This is a file, not folder
.aichats/initial-setup/                  ← NO! Missing date and sequence
.aichats/YYYY-M-DD-1-setup/             ← NO! Must use two-digit format
.aichats/YYYY-MM-DD-01-setup/Opus 4.1--00.md  ← NO! Space in filename
.aichats/YYYY-MM-DD-01-setup/Opus_4.1-00.md   ← NO! Single dash
```
...
0. MASTER INDEX MAINTENANCE (NEW IN v3.2!):
   a. Check if .aichats/INDEX.md exists
...
2. List all folders in .aichats/ directory
...
7. UPDATE MASTER INDEX (MANDATORY!):
   a. Add/update entry for this session in .aichats/INDEX.md
...
The `.aichats/INDEX.md` file is **MANDATORY** and serves as the complete table of contents for ALL sessions. This file MUST be created/updated by EVERY AI assistant.
...
Morning: Claude creates folder and starts work
         → .aichats/YYYY-MM-DD-01-react-setup/Opus-4.1--00.md

Afternoon: User switches to Gemini for specific task
          → .aichats/YYYY-MM-DD-01-react-setup/Gemini-Pro-2.5--00.md

Evening: User asks GPT-4 to review and optimize
         → .aichats/YYYY-MM-DD-01-react-setup/GPT-4-Turbo--00.md
```
...
grep -r "database migration" .aichats/

# Find sessions by date
ls -la .aichats/YYYY-MM-*/

# Search for specific errors across all models
grep -r "TypeError" .aichats/ --include="*.md"

# Find what Gemini worked on
find .aichats -name "Gemini*--*.md" -exec grep -l "API" {} \;
```

### Using with Git
```bash
# View AI chat history
git log --oneline -- .aichats/

# See what was discussed in a commit
git show <commit-hash> -- .aichats/
```
...
### Q: What if .aichats/ folder doesn't exist?
...
```
SESSION START:
□ Check if .aichats/ directory exists
□ 🆕 Check if .aichats/INDEX.md exists (create if missing!)
...
□ 🆕 UPDATE .aichats/INDEX.md with this session's info
